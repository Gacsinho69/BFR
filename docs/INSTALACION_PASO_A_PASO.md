...
> **¿Hay submódulos?** Este repositorio principal no define submódulos de Git. Solo si tu fork muestra un archivo `.gitmodules`
> o detectas carpetas vacías tras el clon, ejecuta inmediatamente después:
> ```bash
...
> **Nota para submódulos**: Si `baufer_modules/baufer_base` proviene de un submódulo Git, confirma que el comando anterior lo copie con su contenido (`ls baufer_modules/baufer_base`). Si ves la carpeta vacía, vuelve al paso 2 y ejecuta `git submodule update --init --recursive`.
